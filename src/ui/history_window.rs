...
use crate::utils::{format_day, format_duration};
...
impl MenuLayerDelegate for HistoryMenu {
    ...
    fn get_num_rows(&self, _menu_layer: MenuLayerRef, section_index: u16) -> u16 {
        if section_index == 0 {
            1
        } else {
    ...
    fn get_cell_height(&self, _menu_layer: MenuLayerRef, index: MenuIndexRef) -> i16 {
        if index.section() == 0 {
            HEADER_HEIGHT
        } else {
            ROW_HEIGHT
    ...
    fn draw_row(&self, ctx: Context, cell_layer: MenuCellLayer, index: MenuIndexRef) {
        if index.section() == 0 {
            ...
            ctx.draw_text(
                &text,
                &font,
                Rect::new(Point::new(0, 15), Size::new(self.bounds_w, 50)),
    ...
    fn select_click(&self, _menu_layer: MenuLayerRef, index: MenuIndexRef) {
        if index.section() == 0 {
            window_manager::push(settings_window::create(), true);
        } else {
            let day_index = index.row() as i32;
            window_manager::push(crate::ui::day_breakdown_window::create(day_index), true);
        }
    }
}
